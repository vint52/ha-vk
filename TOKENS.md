# Как получить все токены и ID VK

В этом файле пошагово описано, как получить все значения VK, которые нужны для настройки `ha-vk`:

- `VK access token`
- `VK peer ID`
- `VK group ID`
- `VK wall access token`

## Для чего нужно каждое значение

- `VK access token`: основной токен сообщества для отправки сообщений, загрузки изображений, видео и публикации постов.
- `VK peer ID`: получатель сообщений. Это может быть ID пользователя или `peer_id` беседы.
- `VK group ID`: числовой ID сообщества, нужен для публикации постов на стену и входящих сообщений сообщества.
- `VK wall access token`: необязательный пользовательский токен, который нужен для фото/видео на стене сообщества и для загрузки видео от имени пользователя. Без него видео в сообщениях можно отправлять как документ, но фото и видео на стену сообщества отправлять нельзя.

## 1. Создайте сообщество VK

Если сообщество уже создано, этот шаг можно пропустить.

1. Откройте `https://vk.com/groups?tab=admin`.
2. Нажмите `Создать сообщество`.
3. Выберите нужный тип и завершите создание.

Это сообщество будет использоваться интеграцией для сообщений и публикации постов.

## 2. Включите сообщения сообщества

1. Откройте настройки сообщества.
2. Перейдите в раздел сообщений.
3. Включите сообщения сообщества.
4. В разделе `Работа с API` -> `Long Poll API` включите long poll сервер для сообщества.

Без этого отправка сообщений через VK API может не работать, а входящие сообщения не будут попадать в Home Assistant.

## 3. Получите `VK access token`

Это основной токен для интеграции.

1. Откройте настройки сообщества.
2. Перейдите в `Работа с API` -> `Ключи доступа`.
3. Нажмите `Создать ключ`.
4. Выдайте нужные права:
   - `Сообщения сообщества` для `messages.send`
   - `Фотографии` для загрузки изображений
   - `Видео` для загрузки видео
   - `Документы`, если хотите загружать видео как документы
   - `Стена`, если хотите публиковать посты
5. Скопируйте созданный токен.
6. Вставьте его в поле `VK access token` при настройке `ha-vk`.

## 4. Получите `VK group ID`

Числовой ID сообщества нужен для публикации постов и входящих сообщений сообщества.

### Вариант 1. Из адреса сообщества

Если адрес сообщества выглядит так:

- `vk.com/club123456789`
- `vk.com/public123456789`

то числовая часть и есть ваш `VK group ID`.

### Вариант 2. Из настроек сообщества

Если у сообщества короткий адрес вместо числового:

1. Откройте настройки сообщества.
2. Найдите числовой ID в разделе основной информации.

### Вариант 3. Через VK API

Также можно посмотреть данные сообщества через:

- `https://vk.com/dev/groups.getById`

## 5. Получите `VK peer ID`

Это значение зависит от того, куда именно вы хотите отправлять сообщения.

### Личные сообщения

Используйте числовой ID пользователя.

Если адрес профиля выглядит как `vk.com/id123456`, то:

- `VK peer ID = 123456`

Если у профиля короткий адрес, числовой ID можно получить через:

- `https://vk.com/dev/users.get`

### Беседы

Для беседы значение рассчитывается так:

- `peer_id = 2000000000 + chat_id`

Как получить `chat_id`:

1. Откройте беседу в веб-версии VK.
2. Посмотрите на адресную строку.
3. Если там есть что-то вроде `im?sel=c123`, тогда:
   - `chat_id = 123`
   - `VK peer ID = 2000000123`

## 6. Получите `VK wall access token`

Этот токен необязателен, но он нужен для фото и видео на стене сообщества, а также для загрузки видео от имени пользователя.

VK позволяет публиковать от имени сообщества по токену сообщества, но фото и видео для стены, а также VK-видео, которые должны загружаться от имени пользователя, требуют пользовательский токен с нужными правами.

### Пошагово

1. Создайте приложение VK типа `Standalone`:
   - `https://vk.com/editapp?act=create`
2. Сохраните приложение и скопируйте его `client_id`.
3. Откройте в браузере ссылку ниже, заменив `CLIENT_ID` на свое значение:

```text
https://oauth.vk.com/authorize?client_id=CLIENT_ID&display=page&redirect_uri=https://oauth.vk.com/blank.html&scope=wall,photos,video,offline&response_type=token&v=5.131
```

4. При необходимости авторизуйтесь в VK и разрешите доступ.
5. После редиректа посмотрите на адресную строку браузера.
6. Найдите в URL фрагмент `access_token=...`.
7. Скопируйте этот токен и используйте его как `VK wall access token`.

Также можно воспользоваться:

- `https://vkhost.github.io/`

Если `vkhost` показывает ошибку вида `{"error":"invalid_request","error_description":"application is blocked"}`, значит выбранное на странице приложение сейчас не выдает нужный пользовательский токен. В таком случае вернитесь назад и попробуйте другое приложение из списка на `vkhost`, например `VK Admin`, `VK Admin (iOS)`, `vk.com` или `Kate Mobile`. Список приложений на сервисе может меняться со временем.

При получении токена обязательно должны быть указаны scope:

- `wall`
- `photos`
- `video`
- `offline`

Пользователь VK, который выдает доступ, должен быть администратором нужного сообщества.

## 7. Какие поля обязательны

- `VK access token`: обязательно
- `VK peer ID`: обязательно
- `VK group ID`: обязателен для постов на стену и входящих сообщений сообщества
- `VK wall access token`: обязателен для фото/видео на стене сообщества и для загрузки видео как VK-видео

Если вы отправляете только сообщения, изображения или видео в личные сообщения и беседы, `VK wall access token` обычно можно не заполнять. Без него видео будут отправляться как документ, если загрузка как VK-видео недоступна.

## Частые проблемы

- `Group authorization failed: method is unavailable with group auth`: для фото/видео на стене сообщества и загрузки видео как VK-видео нужен `VK wall access token`.
- `Входящие сообщения VK недоступны для этой конфигурации сообщества`: проверьте, что в группе включены сообщения сообщества и Long Poll API, а токен сообщества имеет нужные права.
- `VK wall access token is invalid`: получите новый пользовательский токен. Если вы используете `https://vkhost.github.io/` и видите `{"error":"invalid_request","error_description":"application is blocked"}`, попробуйте другое приложение из списка `vkhost`.
- Сообщения уходят не туда: проверьте правильность `VK peer ID`.
- События входящих сообщений не появляются в Home Assistant: проверьте, что настроенный `VK peer ID` точно совпадает с чатом или пользователем, от которого пишет сообщество.
- Публикация поста не работает: проверьте, что `VK group ID` числовой и соответствует нужному сообществу.
