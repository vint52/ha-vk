...
- [x] **Step 1: Write failing tests** for: `_api_call` raising `VkAuthError` with `code` on VK auth error codes (5/15/27/28), `VkNetworkError` on network failure, `async_get_long_poll_server`/`async_check_long_poll` raising `VkLongPollError`, video fallback to document on `VkAuthError(code=27)`, and `is_auth_error` helper.
- [x] **Step 2: Run tests, verify they fail** (`ImportError: cannot import name 'VkAuthError'`).
- [x] **Step 3: Implement** in `api.py`:
...
- [x] **Step 4: Update existing test** `test_send_video_with_invalid_wall_token_raises_clear_error` to raise `VkAuthError("video.save: invalid access token", code=5)`.
- [x] **Step 5: Run full suite, verify pass.**
- [x] **Step 6: Commit** `feat: typed VK API exceptions with error codes`.
...
- [x] **Step 1: Write failing test** that `_async_validate_input` uses `async_get_clientsession(hass)` (patch `config_flow.VkClient`, assert first arg is the shared session).
- [x] **Step 2: Verify fail.**
- [x] **Step 3: Implement:** replace `async_create_clientsession` with `async_get_clientsession`; rewrite `_validation_error_key` to isinstance checks (`VkConfigError` → `invalid_input`/`incoming_not_available` as now; `VkLongPollError` → `incoming_not_available`; `VkAuthError` → `invalid_auth`; `VkNetworkError` → `cannot_connect`; else `vk_error`).
- [x] **Step 4: Update existing test** `test_user_flow_surfaces_incoming_message_setup_errors` to use `VkLongPollError`.
- [x] **Step 5: Run suite, verify pass. Commit** `fix: reuse shared aiohttp session and map validation errors by exception type`.
...
- [x] **Step 1: Write failing test:** plain-text `async_send_message` (with `_api_call` mocked) passes a nonzero `random_id`; two calls produce different values.
- [x] **Step 2: Verify fail. Step 3: Implement** `_random_id()` using `secrets.randbelow(2**31 - 1) + 1`; use in both `messages.send` call sites.
- [x] **Step 4: Run suite, verify pass. Commit** `fix: send random random_id so VK deduplication works`.
...
- [x] **Step 1: Write failing tests:** receiver survives an unexpected `KeyError` from `async_check_long_poll` and keeps emitting events afterwards (patch `asyncio.sleep` or use small delay); receiver calls `entry.async_start_reauth` and stops on `VkAuthError`.
...
- [x] **Step 4: Update existing receiver test** to construct with a `MockConfigEntry` added to hass.
- [x] **Step 5: Run suite, verify pass. Commit** `fix: receiver survives unexpected errors with exponential backoff, triggers reauth`.
...
- [x] **Step 1: Write failing test:** start flow with `SOURCE_REAUTH` context for an existing entry, submit a new token with validation patched, assert `ABORT` with reason `reauth_successful` and updated `entry.data[CONF_VK_ACCESS_TOKEN]`.
- [x] **Step 2: Verify fail. Step 3: Implement** `async_step_reauth` → `async_step_reauth_confirm` (password fields for `vk_access_token` + optional `vk_wall_access_token`), validate merged config, `self.async_update_reload_and_abort(entry, data=..., options=...)`.
- [x] **Step 4: strings/translations:** add `config.step.reauth_confirm` (title, description, data labels), `config.abort.already_configured` and `config.abort.reauth_successful` to `strings.json`, `en.json`, `ru.json` (Russian text for ru).
- [x] **Step 5: Run suite, verify pass. Commit** `feat: reauth flow and config abort strings`.
...
- [x] **Step 1: Update tests first:** services test builds a `MockConfigEntry`, sets `entry.runtime_data = HaVkEntryRuntime(client=client)`, `entry.mock_state(hass, ConfigEntryState.LOADED)`; add test that `hass.data` no longer holds DOMAIN runtimes after setup (or assert `entry.runtime_data.client` set in init test).
- [x] **Step 2: Verify fail. Step 3: Implement:**
...
- [x] **Step 4: Run suite, verify pass. Commit** `refactor: store runtime state in entry.runtime_data`.
...
- [x] **Step 1:** `DEFAULT_API_VERSION = "5.199"`.
- [x] **Step 2: New tests:**
...
- [x] **Step 3: Run suite + ruff, verify pass. Commit** `test: cover notify entity and wall photo posts; bump default VK API version`.

### Task 8: Final verification & changelog

- [x] **Step 1:** `.venv/bin/python -m pytest -q` — all pass.
- [x] **Step 2:** `.venv/bin/ruff check custom_components tests` — clean.
- [x] **Step 3:** Add Unreleased section to `CHANGELOG.md` describing the fixes (session leak, typed errors, receiver backoff/reauth, random_id, runtime_data, reauth flow, strings).
- [x] **Step 4: Commit** `docs: changelog for issue #5 hardening`.
