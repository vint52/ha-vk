...
- Бэкофф: `SEND_RETRY_BACKOFF_BASE = 1.0` секунда, экспоненциально —
  ...
  Экспонента дополнительно ограничена (`2 ** min(attempt, 16)`), чтобы
  `2**attempt` не переполнялся при абсурдных значениях количества повторов.
...
- Идемпотентность `messages.send` обеспечивает существующий `random_id`
  (VK дедуплицирует повторы). `wall.post` получает аналогичный стабильный
  `guid`, генерируемый один раз на вызов `async_send_post`, — иначе потеря
  ответа при таймауте приводила бы к дублю поста.
- Методы VK `photos.saveMessagesPhoto`, `photos.saveWallPhoto`, `docs.save`
  и `video.save` не имеют ключа идемпотентности: если VK принял запись,
  а ответ потерялся, повтор создаст «осиротевший» медиа-объект (пустое
  видео, лишнее фото в скрытом альбоме). Сообщение/пост при этом остаётся
  корректным (одно вложение). Принято как осознанное ограничение —
  у VK нет механизма дедупликации для этих методов.
- В WARNING-лог ретрая download попадает только `netloc + path` URL
  (без query) — query может содержать подписанные токены HA-прокси.
- Суммарная задержка: при полностью недоступной сети падает первый же
  сетевой шаг за `(send_retries + 1) × request_timeout + паузы`; цепочка
  медиа-шагов удлиняет только «мигающие» сценарии, в которых отправка
  в итоге успешна. Общий дедлайн не вводим (YAGNI).
